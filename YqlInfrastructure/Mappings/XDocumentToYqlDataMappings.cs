  ...
  internal static class XDocumentToYqlDataMappings
  {
    public static void CreateIndustryMap()
    {
      ...
      Mapper.CreateMap<XElement, YqlIndustry>()
            .ForMember(dest => dest.Id, opt => opt.ResolveUsing(new Func<XElement,object>(x => Guid.NewGuid())))
      ...
      Mapper.CreateMap<XElement, YqlCompany>()
            ...
            .ForMember(dest => dest.Quoteis, opt => opt.ResolveUsing(new Func<XElement,object>(x => new List<YqlQuote>())))
    ...
    public static void CreateSectorsMap()
    {
      ...
      Mapper.CreateMap<XElement, YqlIndustrySector>()
            .ForMember(src => src.Id, opt => opt.ResolveUsing(new Func<XElement,object>(x => Guid.NewGuid())))
            .ForMember(dest => dest.Name, opt => opt.MapFrom(src => src.Attribute("name").With(x => x.Value)))
    ...
    public static void CreateXChangeRateMap()
    {
      ...
      Mapper.CreateMap<XElement, YqlPair>()
            .ForMember(src => src.Id, opt => opt.ResolveUsing(new Func<XElement,object>(x => Guid.NewGuid())))
            .ForMember(src => src.Rateses, opt => opt.ResolveUsing(new Func<XElement,object>(x => new List<YqlXchangeRate>())))
      ...
      Mapper.CreateMap<XElement, YqlXchangeRate>()
            ...
            .ForMember(src => src.Pair, opt => opt.ResolveUsing(new Func<XElement,object>(Mapper.Map<YqlPair>)))
